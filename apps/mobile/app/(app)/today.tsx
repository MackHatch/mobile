...
export default function TodayScreen() {
  ...
  const fetchFromApi = useCallback(async () => {
    ...
    try {
      ...
      upsertHabitsFromApi(habitsRes.habits);
      const checkinRes = await apiFetch<{
        date: string;
        mood?: number;
        completions?: Array<{ habitId: string; done: boolean }>;
      }>(`/api/checkins/${selectedDate}`, { token }).catch(() => null);
      if (checkinRes?.date === selectedDate) {
        upsertCheckinFromApi(selectedDate, {
          mood: checkinRes.mood,
          completions: checkinRes.completions,
        });
      }
      loadLocal();
    } catch {
